...
rockpaperscissors=$(npx hardhat starknet-deploy --starknet-network $1 --wait starknet-artifacts/contracts/RockPaperScissors.cairo/)
address=$(echo -e "$rockpaperscissors" | grep "Contract address" | awk 'NF{print $NF}')
echo '{ "address": "'$address'" }' > starknet-deployments/$1/RockPaperScissors.json